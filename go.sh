...
# go.sh — launch moon-lit for local development.
#
...
#                        test things like the launcher's "Export to File".
...
run_arcade() {
  require_launcher
  echo "→ Launching moon-lit inside the arcade framework via $LAUNCHER_DIR/dev.sh"
  ...
  # moon-lit tile loads the game in its iframe with the full SDK handshake.
